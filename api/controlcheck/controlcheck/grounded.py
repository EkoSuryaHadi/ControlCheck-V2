...
MODELS: tuple[str, ...] = ('qwen3.7-flash-2026-07-15', 'qwen3.8-flash', 'gpt-5-mini', 'gpt-4o-mini', 'claude-haiku-4-5', 'kimi-k2', 'gemini/gemini-2.5-pro', 'gemini/gemini-2.0-flash')


class SumoPodGateway:
    def __init__(self, api_key=None, model=None, client=None, base_url=None):
        self.api_key = api_key or os.getenv('SUMOPOD_API_KEY') or os.getenv('OPENAI_API_KEY')
        self.model = model or os.getenv('SUMOPOD_MODEL', 'gpt-5-mini')
        self.base_url = (base_url or os.getenv('SUMOPOD_BASE_URL', SUMOPOD_BASE_URL)).rstrip('/')
    ...
    def complete(self, messages, model=None):
        ...
        allowed = set(MODELS)
        if self.model:
            allowed.add(self.model)
        if not self.enabled or selected not in allowed:
            raise ValueError('Provider SumoPod belum siap.')
        response = self.client.post(self.base_url + '/chat/completions', headers={'Authorization': f'Bearer {self.api_key}'},
...
class GroundedAssistant:
    ...
    def answer(self, question, snapshot, model=None, comparison=None, forecast_readiness=None):
        result = analyze(snapshot['rows'], snapshot['as_of'])
        allowed = {_token(row['evidence']): row['evidence'] for row in snapshot['rows']}
        bounded_comparison = None
        if comparison:
            bounded_comparison = {
                'previous_snapshot': comparison.get('previous_snapshot'),
                'current_snapshot': comparison.get('current_snapshot'),
                'metrics': comparison.get('metrics'),
                'activities': {k: (v[:20] if isinstance(v, list) else v) for k, v in comparison.get('activities', {}).items()},
        ...
        context = {'snapshot': {'version': snapshot['version'], 'as_of': snapshot['as_of'], 'currency': snapshot['currency']},
                   'metrics': result['metrics'], 'comparison': bounded_comparison, 'forecast_readiness': forecast_readiness,
                   'recovery_priorities': [
                       {key: (_token(value) if key == 'citation' else value) for key, value in item.items()}
                       for item in result['recovery_priorities'][:30]],
                   'insights': [
                       {key: value for key, value in insight.items() if key != 'evidence'} |
                       {'citations': [_token(evidence) for evidence in insight['evidence'][:10]]}
