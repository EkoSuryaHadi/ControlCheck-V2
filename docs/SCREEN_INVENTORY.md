...
| Data Center | Convert source into approved data | File and sheet selection, header mapping, preview findings, publish | Upload progress state; parse failures; blocking quality findings; missing optional fields |
...
Data Center must display suggestion confidence/rationale, a field mapping editor, source preview, row count, selected sheet, source ID and publication policy. Publication must never be enabled before a successful quality check of the current mapping. Changing mapping invalidates that check.
