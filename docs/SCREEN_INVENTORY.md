...
| Data Center | Convert project files into approved data | Upload CSV/XLSX/MPP/XML/Primavera P6 XER; agent maps, validates and publishes; advanced review is optional | Agent progress, decision receipt, parse failures, blocking quality findings and review action |
...
Data Center starts with one multi-file upload action and shows the agent receipt: source decisions, coverage, issues and snapshot version. Advanced review retains the inspected matrix, normalization settings, mapping editor, source preview and manual publication policy when automatic intake needs correction.

## Follow-up screens
Mapping templates, quality resolution editor, source evidence drawer, report templates, forecasting assumptions/results, agent action approvals and workspace membership. These are designed as roadmap boundaries, not represented as functional screens in this scaffold.
