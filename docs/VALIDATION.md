...
Verified 9 September 2026 on Windows, Python 3.11 and Node.js 24.3.0.

## Automated checks

- `.venv/Scripts/python.exe -m pytest tests -q` → **21 passed**, 2 dependency deprecation warnings.
...
The request journey is also covered in `tests/test_api.py`: create, inspect, explicit header/normalization upload, mapping review, quality validation, publish, overview metrics, assistant answer, report text, idempotent republish, project isolation, restart persistence, invalid XLSX formula rejection and unsupported MPP/XER response.

## Known limitations

There is no external AI provider in this increment; answers are explicitly labelled local analytics. Original file bytes are not retained, and locale handling intentionally requires an explicit user choice. Use loopback-only development until the production gates in `docs/ARCHITECTURE.md` are implemented.
