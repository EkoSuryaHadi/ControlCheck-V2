...
2. Upload Excel (.xlsx) or UTF-8 CSV. Select a worksheet before upload if needed.
...
| Upload | CSV/XLSX, clear constraints, sheet choice, immutable source identity | CSV/XLSX upload, explicit sheet name, 5 MB / 10,000 rows / 100 columns, SHA-256 source identity |
...
One row is one leaf activity, one reporting snapshot, one currency. Required fields: activity_id, name. Optional: planned_start, planned_finish (ISO YYYY-MM-DD), planned_progress, actual_progress (0–100 percentage points), budget, actual_cost (non-negative numbers), weight (positive). Never mix WBS summary rows with leaf activity rows. Values such as 0.5 mean 0.5%, not 50%; conversion requires explicit upstream preparation in this increment. Currency symbols and locale-specific thousands separators are rejected, not guessed. Excel formula cells must be replaced with values. Missing values stay unknown.

Each publication is a complete replacement snapshot, not an append or merge. This avoids double counting repeated activity IDs and costs. Separate schedule/cost/progress table reconciliation is full-MVP follow-up. Workbook files with multiple sheets default to the first sheet; other sheets require explicit selection by name. Forecast dates, critical path and root-cause claims cannot be inferred from this contract.
