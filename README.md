...
Use one complete leaf-activity snapshot per upload. A new publication replaces the active view rather than merging files. Do not combine WBS summaries and their child rows. CSV is UTF-8 with comma, semicolon or tab delimiter. Data Center previews the first rows, suggests a header row, lists XLSX sheets and requires explicit date (`YYYY-MM-DD`, `DD/MM/YYYY` or `MM/DD/YYYY`), decimal and progress-scale choices. Formula cells must first be converted to values. Currency: one project currency; no symbols or thousands separators in numeric cells. Limits: CSV/XLSX 5 MB and 10,000 rows; MPP/XER 50 MB and 25,000 activities; 100 columns.
