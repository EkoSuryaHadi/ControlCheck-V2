...
function FormattedAnswer({ text }: { text: string }) {
  let normalized = text;
  if (!normalized.includes('\n\n') && /\b1\.\s+/.test(normalized)) {
    normalized = normalized.replace(/:\s+1\.\s+/, ':\n\n1. ');
    normalized = normalized.replace(/\s+(\d+\.\s+)/g, '\n\n$1');
    normalized = normalized.replace(/\s+(Planner:|Site Team:|Output review:)/g, '\n• **$1**');
  }
  ...
  return (
    <div className="answer-content">
      {paragraphs.map((para, idx) => {
        ...
        const isCard = /^\d+\.\s+/.test(firstLine.trim());

        if (isCard) {
          return (
            <div key={idx} className="answer-card">
              <div className="answer-card-header">{renderInlineBold(firstLine)}</div>
              {lines.slice(1).map((line, lIdx) => {
                const trimmed = line.trim();
                const isBullet = trimmed.startsWith('•') || trimmed.startsWith('-');
                const content = isBullet ? trimmed.replace(/^[•\-]\s*/, '') : line;
                return (
                  <div key={lIdx} className="answer-bullet">
        ...
        return (
          <div key={idx} className={idx === 0 && paragraphs.length > 1 ? 'answer-intro' : 'answer-para'}>
            {lines.map((line, lIdx) => {
              const trimmed = line.trim();
              const isBullet = trimmed.startsWith('•') || trimmed.startsWith('-');
              if (isBullet) {
              ...
              return <p key={lIdx} className="answer-line">{renderInlineBold(line)}</p>;
            })}
          </div>
        );
      })}
...
function Delta({ value, suffix='' }: { value: number | null; suffix?: string }) {
  ...
  return <span className={value > 0 ? 'delta positive' : value < 0 ? 'delta negative' : 'delta'}>{value > 0 ? '+' : ''}{number(value)}{suffix}</span>;
}

function ComparisonCard({ comparison }: { comparison: SnapshotComparison }) {
  const activities = comparison.activities;
  return <section className="comparison"><div className="section-title"><div><h2>Perubahan sejak laporan sebelumnya</h2><p className="muted">v{comparison.previous_snapshot.version} · {comparison.previous_snapshot.as_of} → v{comparison.current_snapshot.version} · {comparison.current_snapshot.as_of}</p></div><span className="tag">2 SNAPSHOT</span></div><div className="comparison-metrics"><div><span>Progress</span><strong><Delta value={comparison.metrics.progress_delta} suffix="%"/></strong></div><div><span>SPI</span><strong><Delta value={comparison.metrics.spi_delta}/></strong></div><div><span>CPI</span><strong><Delta value={comparison.metrics.cpi_delta}/></strong></div><div><span>Terlambat</span><strong><Delta value={comparison.metrics.overdue_delta}/></strong></div></div><div className="change-list"><p><b>{activities.newly_overdue_ids.length}</b> baru terlambat · <b>{activities.resolved_overdue_ids.length}</b> tidak lagi terlambat · <b>{activities.progress_changed_ids.length}</b> progress berubah</p>{(activities.added_ids.length || activities.removed_ids.length) ? <p className="muted">Cakupan sumber: {activities.added_ids.length} aktivitas ditambah, {activities.removed_ids.length} dihapus. Ini tidak otomatis berarti perubahan kinerja.</p> : null}</div><details className="limitations"><summary>Batasan perbandingan</summary>{comparison.limitations.map(item=><p key={item}>{item}</p>)}</details></section>;
}

function ForecastReadinessCard({ readiness }: { readiness: ForecastReadiness }) {
  const status = {
    not_ready: { label: 'Belum siap untuk forecast', tone: 'missing' },
    needs_attention: { label: 'Perlu perbaikan sebelum forecast', tone: 'attention' },
    ready_for_method: { label: 'Siap memilih metode forecast', tone: 'ready' },
  }[readiness.status];
...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="brandbar"><a className="wordmark" href="#"><b className="brand-symbol" aria-hidden="true">✓</b>ControlCheck<span>AI</span></a><span className="edition">PROJECT INTELLIGENCE / 2.0</span><span className="local-status"><i/> Project intelligence</span></header>
    ...
    <footer>ControlCheck AI 2.0 <span>Data → Context → Intelligence</span><span>Berbasis data · Dapat ditelusuri</span></footer>
  </div>;
}
...
function Workspace({project}: {project: Project}) {
  ...
  const [reportingDate,setReportingDate] = useState(project.as_of);
  ...
  async function publish() { if(source) await run('Menerbitkan snapshot…',async()=>{
    await api(base+'/sources/'+source.id+'/publish',json({mapping,as_of:reportingDate})); await refresh(); setAnswers([]);setReport('');
  ...
  function reconciliationPayload() { return {schedule_source_id:scheduleId, schedule_mapping:automaticMapping(scheduleId), as_of:reportingDate, ...(progressId ? {progress_source_id:progressId,progress_mapping:automaticMapping(progressId)} : {}), ...(costId ? {cost_source_id:costId,cost_mapping:automaticMapping(costId)} : {})}; }
  ...
  return <div className="workspace"><aside><p className="eyebrow">PROJECT WORKSPACE</p><nav aria-label="Navigasi proyek">{tabs.map((name,i)=><button key={name} className={tab===name?'active':''} aria-current={tab===name?'page':undefined} onClick={()=>{setTab(name);setError('');}}><span className="nav-number">0{i+1}</span>{tabLabels[name]}{name==='Insights' && !!analysis?.insights.length && <em>{analysis.insights.length}</em>}</button>)}</nav><div className="sidebar-note"><span className="tag">{aiModels?.enabled ? 'GROUNDED AI' : 'ANALITIK LOKAL'}</span><p>Setiap keputusan dimulai dari data.</p><small>Jawaban merujuk snapshot dan sumber proyek.</small></div></aside>
    <main><div className="page-heading"><div><p className="eyebrow">{project.name} / {project.currency}</p><h1>{tab === 'Overview' ? 'Kendali proyek Anda.' : tabLabels[tab]}</h1></div><div className="heading-actions"><button className="secondary" onClick={()=>setTab('Data Center')}>↑ Upload data</button><button onClick={()=>setTab('AI Assistant')}>Tanya AI ↗</button></div><div className="snapshot-label">Tanggal data <strong>{snapshot?.as_of || project.as_of}</strong>{snapshot ? <span>Snapshot v{snapshot.version}</span> : <span>Belum dipublikasikan</span>}</div></div>
      ...
      {tab==='Overview' && (!snapshot || !metrics ? empty : <>
        ...
        {metrics.critical_count != null && <section className="budget-strip"><h2>Analisis jadwal <small>fakta dari file schedule</small></h2><dl><div><dt>Aktivitas critical</dt><dd>{metrics.critical_count}</dd><small>Cakupan {metrics.critical_coverage}</small></div><div><dt>Critical terlambat</dt><dd>{metrics.critical_overdue}</dd></div><div><dt>Milestone terlambat</dt><dd>{metrics.milestone_overdue}</dd><small>Cakupan {metrics.milestone_coverage}</small></div><div><dt>Jalur dampak</dt><dd>{number(metrics.downstream_activities,0)}</dd><small>Cakupan {metrics.dependency_coverage}</small></div><div><dt>Data float</dt><dd>{metrics.slack_coverage}</dd></div></dl></section>}
      ...
      {tab==='AI Assistant' && (!snapshot ? empty : <section className="chat"><div className="chat-intro"><span className="tag">{aiModels?.enabled ? 'SUMOPOD · GROUNDED AI' : 'ANALITIK LOKAL'} · SNAPSHOT v{snapshot.version}</span><h2>Apa yang ingin Anda<br/>pahami tentang proyek ini?</h2><p>Jawaban memakai perhitungan dan sumber yang telah dipublikasikan.</p>{aiModels?.enabled && <label>Model SumoPod<select value={model} onChange={e=>setModel(e.target.value)}>{aiModels.models.map(item=><option key={item}>{item}</option>)}</select></label>}</div><div className="prompts">{['Bagaimana kondisi proyek saya?','Apa yang berubah sejak laporan sebelumnya?','Apakah data saya siap untuk forecast?','Aktivitas apa yang terlambat?','Ringkas progress dan biaya proyek.'].map(q=><button key={q} className="secondary" disabled={!!busy} onClick={()=>void ask(q)}>{q} ↗</button>)}</div><div className="conversation" aria-live="polite">{answers.length > 3 && <div style={{textAlign:'center',margin:'18px 0 10px'}}><button type="button" className="secondary" style={{fontSize:'11px',padding:'7px 16px',borderRadius:'20px'}} onClick={()=>setShowAllHistory(prev=>!prev)}>{showAllHistory ? '▲ Sembunyikan riwayat lama (tampilkan 3 terakhir)' : `▾ Tampilkan ${answers.length - 3} percakapan sebelumnya`}</button></div>}{(showAllHistory ? answers : answers.slice(-3)).map((a,i)=><article key={i}><p className="user-question">{a.question}</p><div className="answer"><span className="eyebrow">CONTROLCHECK / {a.response.mode}</span><FormattedAnswer text={a.response.answer}/><EvidenceList evidence={a.response.evidence}/><details><summary>Keterbatasan jawaban</summary>{a.response.limitations.map(l=><p key={l}>{l}</p>)}</details></div></article>)}</div><form className="composer" onSubmit={e=>{e.preventDefault();void ask();}}><label htmlFor="question">Pertanyaan proyek</label><textarea id="question" value={question} onChange={e=>setQuestion(e.target.value)} placeholder="Tanyakan progress, biaya, atau keterlambatan…" maxLength={2000} required/><button disabled={!!busy || !question.trim()}>Analisis pertanyaan →</button></form><p className="hint">Jawaban AI hanya memakai snapshot yang disetujui dan citation yang tervalidasi.</p></section>)}
      ...
      {tab==='Settings' && <section><p className="lead">Fondasi yang transparan untuk pengembangan berikutnya.</p><dl className="settings"><div><dt>Nama proyek</dt><dd>{project.name}</dd></div><div><dt>Project ID</dt><dd>{project.id}</dd></div><div><dt>Mata uang / tanggal data</dt><dd>{project.currency} / {project.as_of}</dd></div><div><dt>Assistant</dt><dd>{aiModels?.enabled ? 'SumoPod Grounded AI — citation wajib' : 'Analitik lokal'}</dd></div><div><dt>Schema mapping</dt><dd>Alias kolom + konfirmasi manual</dd></div><div><dt>Format aktif</dt><dd>CSV, XLSX, MPP, XML, XER</dd></div><div><dt>Fondasi lanjutan</dt><dd>MPP/XER · Forecasting · Agent action proposals</dd></div><div><dt>Penyimpanan</dt><dd>SQLite lokal · snapshot versi terpisah per proyek</dd></div></dl><p className="hint">Setiap publikasi menyimpan tanggal laporannya sendiri dan Overview membandingkan dengan snapshot sebelumnya. Autentikasi multiuser dan pengaturan provider di aplikasi belum diaktifkan.</p></section>}
      </>}
    </main>
  </div>;
}
